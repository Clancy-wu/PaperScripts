...
def compute_glm_results(sub_file):
    ...
    sub_motion_36P = sub_motion.loc[:, column_36P]
    #####################################################
    ##    Avg response
    #####################################################
    ## load time event: run-01 is rested wakefulness (RW), run-02 is sleep deprivation (SD)
    if sub_run == 'run-01':
        time_event_file = f'STWMdata/STWM_time_event/{sub_name}/Before/time_event_avg_removeTenTimePoint.csv'
    elif sub_run == 'run-02':
        time_event_file = f'STWMdata/STWM_time_event/{sub_name}/After/time_event_avg_removeTenTimePoint.csv'
    else:
        pass
    ## do GLM
    t_r = 2.02
    ...
    sub_glm.fit(run_imgs=sub_file, events=time_event, confounds=sub_motion_36P)
    sub_MemoryControl = sub_glm.compute_contrast("memory - control", output_type='z_score')
    sub_MemoryControl_out = f'STWMdata/AvgActive/{sub_name}_task-rdm_{sub_run}_con_z.nii.gz'
    ...
    ## load time event 
    if sub_run == 'run-01':
        time_event_file = f'STWMdata/STWM_time_event/{sub_name}/Before/time_event_digit_removeTenTimePoint.csv'
    elif sub_run == 'run-02':
        time_event_file = f'STWMdata/STWM_time_event/{sub_name}/After/time_event_digit_removeTenTimePoint.csv'
    else:
        pass
    ## do GLM
    t_r = 2.02
    ...
    sub_NumOne_out = f'STWMdata/NumOne/{sub_name}_task-rdm_{sub_run}_con_z.nii.gz'
    ...
    sub_NumThree_out = f'STWMdata/NumThree/{sub_name}_task-rdm_{sub_run}_con_z.nii.gz'
    ...
    sub_NumSix_out = f'STWMdata/NumSix/{sub_name}_task-rdm_{sub_run}_con_z.nii.gz'
